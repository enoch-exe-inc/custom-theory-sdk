...
var id = "custom_diffcalc"
var name = "Ext. Differential Calculus";
var description = "An extension of the 'Differential Calculus' theory from the game.";
...
var init = () => {
	...
	// q1
	{
		...
		dq1 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(5, Math.log2(2))));
	...
	// q2
	{
		...
		dq2 = theory.createUpgrade(1, currency, new ExponentialCost(2500, Math.log2(2)));
	...
	// q3
	{
		...
		dq3 = theory.createUpgrade(2, currency, new ExponentialCost(1.5e20, Math.log2(3)));
	...
	// q4
	{
		...
		dq4 = theory.createUpgrade(3, currency, new ExponentialCost(4e40, Math.log2(4)));
	...
	// r1
	{
		...
		dr1 = theory.createUpgrade(4, currency, new ExponentialCost(1e3, Math.log2(2)));
	...
	// r2
	{
		...
		dr2 = theory.createUpgrade(5, currency, new ExponentialCost(3e6, Math.log2(2)));
	...
	// r3
	{
		...
		dr3 = theory.createUpgrade(6, currency, new ExponentialCost(2e20, Math.log2(3)));
	...
	// r4
	{
		...
		dr4 = theory.createUpgrade(7, currency, new ExponentialCost(2.5e40, Math.log2(4)));
	...
	theory.createPublicationUpgrade(0, currency, 1e9);	// 1e15
	theory.createBuyAllUpgrade(1, currency, 1e15);		// 1e18
	theory.createAutoBuyerUpgrade(2, currency, 1e30);	// 1e35

	//////////////////////
	// Checkpoint Upgrades
	theory.setMilestoneCost(new LinearCost(0, 2.0));	// Original: (25, 25) - Gain 1 milestone upgrade per 1e25 of tau
	...
	{
		r1Exp = theory.createMilestoneUpgrade(3, 5);
...
var tick = (elapsedTime, multiplier) => {
	...
	currency.value += bonus * dt * (q1.pow(getQ1Exp(q1Exp.level)) * r1.pow(getR1Exp(r1Exp.level)));
...
var getPublicationMultiplier = (tau) => tau.isZero ? (BigNumber.ONE * sigma.pow(getSig(multSig.level))) : (tau.pow(1.2) * sigma.pow(getSig(multSig.level))); // Original: tau.pow(0.198) / BigNumber.HUNDRED;
var getPublicationMultiplierFormula = (symbol) => "\\left(\\frac{{\\sigma_{t}}}{20}\\right)^{" + getSig(multSig.level).toString(0) + "} {" + symbol + "}^{1.2}";	// Original: "\\frac{{" + symbol + "}^{0.198}}{100}";
