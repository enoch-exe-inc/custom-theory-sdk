...
var init = () => {
	...
	// c4
	{
		...
		c4 = theory.createUpgrade(5, currency, new ExponentialCost(1e8, 8 * Math.log2(10)));	// Original: (1e10, 8 * Math.log2(10))
...
var getPublicationMultiplier = (tau) => (game.sigmaTotal / 20) * (tau.pow(0.2) / BigNumber.TWO);	// Original: tau.pow(0.164) / BigNumber.THREE
var getPublicationMultiplierFormula = (symbol) => "\\left(\\frac{{\\sigma_{t}}}{20}\\right) \\frac{{" + symbol + "}^{0.2}}{2}";	// Original: "\\frac{{" + symbol + "}^{0.15}}{2}";
